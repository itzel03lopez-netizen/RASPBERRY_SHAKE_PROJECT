...
COOLDOWN_EVENTO = 40   # segundos de espera entre detecciones
...
def init_dirs():
    ...
    # Sube logos institucionales a Render para que aparezcan en la pagina en la nube
    subir_logos_a_render()
...
def _post_a_render(endpoint, data=None, files_bytes=None, timeout=30):
    """
    Realiza un POST a Render en un hilo de fondo.
    Recibe los bytes de los archivos ya leídos para evitar problemas de
    descriptores de archivo en hilos concurrentes.

    timeout largo (30 s) para tolerar el cold-start del plan gratuito (~15-25 s).
    """
    try:
        files = {}
        if files_bytes:
            for nombre, (fname, data_bytes, mime) in files_bytes.items():
                import io
                files[nombre] = (fname, io.BytesIO(data_bytes), mime)
        ...
        if resp.status_code == 200:
            log.debug('Render %s OK', endpoint)
        else:
            log.warning('Render %s → %s: %s', endpoint, resp.status_code, resp.text[:80])
    except requests.exceptions.ConnectionError:
        log.debug('Sin conexión a internet — POST %s omitido', endpoint)
    except requests.exceptions.Timeout:
        log.warning('Timeout enviando a Render %s', endpoint)
    except Exception as e:
        log.warning('Error enviando a Render %s: %s', endpoint, str(e))


def upload_a_render():
    """
    Envía el sismograma actual y el estado del sistema al servidor en Render.
    Se llama automáticamente cada 5 s al final de generate_plot().

    Corre en un hilo de fondo para no bloquear el hilo de procesamiento
    (el cold-start de Render free puede tardar hasta 30 s).
    """
    # Lee el estado bajo el lock ANTES de lanzar el hilo
    with lock:
        estado_actual = {
    ...
    # Lee los bytes de los archivos aquí (hilo principal) para evitar condiciones
    # de carrera si generate_plot sobrescribe el archivo mientras el hilo corre
    files_bytes = {}
    img_path = WEB_STATIC_DIR / 'sismograma.png'
    if img_path.exists():
        try:
            files_bytes['sismograma'] = ('sismograma.png',
                                         img_path.read_bytes(),
                                         'image/png')
        except Exception:
            pass
    ...
    threading.Thread(
        target=_post_a_render,
        args=('/update_data', estado_actual, files_bytes),
        daemon=True,
    ).start()


def subir_logos_a_render():
    """
    Sube los logos institucionales a Render una sola vez al arrancar.
    Render los guarda en /tmp y los sirve en /logo/enes y /logo/geo,
    de modo que la página en la nube muestre los mismos logos que la laptop.
    """
    files_bytes = {}
    if LOGO_ENES.exists():
        try:
            files_bytes['logo_enes'] = ('logo_enes.png', LOGO_ENES.read_bytes(), 'image/png')
        except Exception:
            pass
    if LOGO_GEO.exists():
        try:
            files_bytes['logo_geo'] = ('logo_geo.png', LOGO_GEO.read_bytes(), 'image/png')
        except Exception:
            pass

    if not files_bytes:
        log.warning('No se encontraron logos para subir a Render.')
        return

    threading.Thread(
        target=_post_a_render,
        args=('/update_logos', None, files_bytes),
        daemon=True,
    ).start()
    log.info('Subiendo logos institucionales a Render...')
...
<body>
  ...
  <!-- Modal con el codigo QR de la URL publica -->
  <div class="qr-overlay" id="qr-overlay" onclick="if(event.target===this)this.classList.remove('open')">
    <div class="qr-modal">
      <h2>Ver en otro dispositivo</h2>
      <div style="display:flex;flex-direction:column;align-items:center;gap:10px;">
        <p style="font-size:12px;color:#8b949e;">Escanea para abrir el sismografo</p>
        <div class="qr-frame">
