...
def _wifi_escape(texto):
    """
    Escapa los caracteres especiales requeridos por el estandar de QR WiFi (IEEE 802.11).
    Los caracteres  \\  ;  ,  "  :  deben ir precedidos de una barra invertida.
    Sin este escape, el celular recibe una cadena mal formada y pide la contrasena de nuevo.
    """
    result = ''
    for c in texto:
        if c in ('\\', ';', ',', '"', ':'):
...
def generar_qr():
    """
    Genera dos imagenes QR en webapp/static/:
      - url_qr.png  : URL del sismografo (http://IP:5000)
      - wifi_qr.png : cadena WiFi para conectarse al hotspot del Pi automaticamente

    Formato estandar WiFi QR: WIFI:T:WPA;S:<ssid>;P:<pass>;;
    Los caracteres especiales se escapan correctamente para evitar errores de autenticacion.
    Requiere: pip install qrcode[pil]
    """
    ip = get_local_ip()
    try:
        url      = f'http://{ip}:{WEB_PORT}'
        # Escapa SSID y contrasena antes de armar la cadena
        ssid_esc = _wifi_escape(WIFI_SSID)
        pass_esc = _wifi_escape(WIFI_PASSWORD)
        wifi_str = f'WIFI:T:WPA;S:{ssid_esc};P:{pass_esc};;'
        _hacer_qr(url,      WEB_STATIC_DIR / 'url_qr.png')
        _hacer_qr(wifi_str, WEB_STATIC_DIR / 'wifi_qr.png', box_size=5)
        log.info('QR generado — URL: %s  |  WiFi SSID: %s', url, WIFI_SSID)
    except ImportError:
        log.warning('qrcode no instalado. Ejecuta con internet: pip install qrcode[pil]')
...
def generate_plot(trace):
    ...
    tr_filtrado.filter('bandpass', freqmin=1.0, freqmax=20.0, corners=4, zerophase=True)
    ...
    <div class="qr-modal">
      ...
      <div class="qr-pair">
        ...
        <div class="qr-item">
          ...
          <div class="qr-url">http://{{ local_ip }}:{{ web_port }}</div>
        </div>
      </div>
...
@app.route('/qr/wifi')
def servir_qr_wifi():
    """Sirve el QR de conexion WiFi al hotspot del Pi."""
