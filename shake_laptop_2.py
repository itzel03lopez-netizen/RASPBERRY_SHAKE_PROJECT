...
PAQUETES = ['obspy', 'matplotlib', 'Pillow', 'flask', 'qrcode[pil]']
...
# Debe ser mayor que VENTANA_SEG (30 s) para garantizar que el buffer ya no
# contiene la señal del evento anterior cuando la deteccion se reactiva.
...
# URL local del sismografo (se actualiza en init_dirs una vez que se detecta la IP)
# Formato: http://<IP-laptop-en-hotspot-Pi>:<WEB_PORT>
URL_LOCAL = 'http://localhost:' + str(5000)   # valor provisional; se reemplaza al arrancar
...
def get_local_ip():
    """
    Detecta la IP de la laptop en la misma red que el RS1D.

    Truco: abrir un socket UDP "falso" hacia la IP del Pi hace que el sistema
    operativo seleccione EXACTAMENTE la interfaz de red que puede alcanzar al Pi,
    aunque haya varias interfaces activas (WiFi del Pi + red de la escuela, etc.).
    No se envian datos reales.
    """
    try:
        s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        s.settimeout(1)
        s.connect((SHAKE_IP, 18000))   # IP real del Pi, no el gateway generico
        ip = s.getsockname()[0]
        s.close()
        return ip
    except Exception:
        # Fallback: primer IP no-loopback disponible
        try:
...
def generar_qr():
    """
    Genera dos QRs:
      1) WiFi  → para que otros dispositivos se conecten al hotspot del Pi
      2) URL   → para que abran el sismografo una vez conectados
    """
    global URL_LOCAL
    ip = get_local_ip()
    URL_LOCAL = 'http://' + ip + ':' + str(WEB_PORT)

    try:
        # QR 1: cadena estándar de configuración WiFi (reconocida por iOS y Android)
        wifi_str = f'WIFI:T:WPA;S:{WIFI_SSID};P:{WIFI_PASSWORD};;'
        _hacer_qr(wifi_str, WEB_STATIC_DIR / 'wifi_qr.png')

        # QR 2: URL local del sismógrafo
        _hacer_qr(URL_LOCAL, WEB_STATIC_DIR / 'url_qr.png')

        log.info('QRs generados — WiFi: %s | URL: %s', WIFI_SSID, URL_LOCAL)
    except ImportError:
...
def init_dirs():
    ...
    # Genera el codigo QR con la URL local del servidor
    global LOCAL_IP
    LOCAL_IP = generar_qr()
...
def generate_plot(trace):
    ...
    # Copia la imagen al directorio web para que el servidor Flask la sirva
    shutil.copy(str(PLOT_PATH), str(WEB_STATIC_DIR / 'sismograma.png'))
    log.info('Sismograma actualizado.')
                ...
                if cooldown_ok:
                    ...
                    def _reproducir_alarma():
                        try:
                            ...
                            # MUY IMPORTANTE: reinicia el cooldown desde AHORA (no desde
                            # cuando ocurrio el evento) para que el buffer de 30 s tenga
                            # tiempo de rodar antes de la siguiente deteccion.
                            with lock:
                                estado['alarma_activa']  = False
                                estado['ultimo_evento_t'] = time.time()
...
<body>
  ...
  <header>
    ...
    <div style="display:flex;flex-direction:column;align-items:flex-end;gap:6px;">
      ...
      <button class="btn-qr" id="btn-activar-sonido" onclick="desbloquearSonido()" title="Activar alertas de sonido en este dispositivo">&#128276; Activar alertas</button>
    </div>
  </header>

  <!-- Modal con dos QRs: WiFi del Pi y URL del sismografo -->
      ...
      <div class="qr-pair">
        ...
        <div class="qr-item">
          ...
          <div style="font-size:11px;color:#8b949e;margin-top:6px;">
            Red: <b style="color:#e6edf3">{{ wifi_ssid }}</b>
          </div>
        </div>
        ...
        <div class="qr-item">
          ...
          <div class="qr-url">{{ url_local }}</div>
        </div>
  ...
  <script>
    // ── Alarma de audio ───────────────────────────────────────────────────────
    // Los navegadores móviles bloquean el audio automático hasta que el usuario
    // toca la pantalla al menos una vez. desbloquearSonido() se llama al presionar
    // el botón "Activar alertas", lo que "desbloquea" el contexto de audio y permite
    // que la alarma suene automáticamente en eventos futuros sin más interacción.
    const audioAlarma = new Audio('/alarma');
    audioAlarma.preload = 'auto';

    let alertaSonando      = false;  // evita solapamiento si el sonido ya está corriendo
    let sonidoDesbloqueado = false;  // true una vez que el usuario tocó el botón
    let eventoAnterior     = false;  // para detectar el flanco de subida (false → true)

    function desbloquearSonido() {
      // Reproduce y pausa inmediatamente: esto "desbloquea" el audio en el navegador
      audioAlarma.play()
        .then(() => {
          audioAlarma.pause();
          ...
          btn.onclick = null;   // ya no hace falta volver a tocar
        })
        .catch(() => {
          // Algunos navegadores muy restrictivos fallan aquí; el usuario puede intentar de nuevo
          alert('No se pudo activar el sonido. Intenta de nuevo o revisa los permisos de audio del navegador.');
        });
    }

    function reproducirAlarma() {
      if (!sonidoDesbloqueado || alertaSonando) return;
      alertaSonando = true;
      audioAlarma.currentTime = 0;
      audioAlarma.play().catch(e => console.warn('Audio:', e));
      ...
      fetch('/estado')
        ...
        .then(d => {
          ...
          if (!d.conectado) {
            ...
            // Solo dispara el sonido en el FLANCO (cuando el evento aparece por primera vez).
            // Sin esto, reproducirAlarma() se llamaría cada 3 s mientras dure el evento.
            if (!eventoAnterior) reproducirAlarma();
          } else {
            b.textContent = 'En vivo'; b.className = 'badge';
            banner.classList.remove('visible');
          }
          eventoAnterior = d.evento_activo;   // guarda el estado para el siguiente ciclo
        })
...
def index():
    ...
    # render_template_string reemplaza {{ station }}, {{ url_local }}, etc.
    return render_template_string(HTML, station=STATION, local_ip=LOCAL_IP,
                                  web_port=WEB_PORT, url_local=URL_LOCAL,
                                  wifi_ssid=WIFI_SSID)
...
@app.route('/qr/wifi')
def servir_qr_wifi():
    """Sirve el QR con las credenciales del hotspot del Pi."""
    f = WEB_STATIC_DIR / 'wifi_qr.png'
