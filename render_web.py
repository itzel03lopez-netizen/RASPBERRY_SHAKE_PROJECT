...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .placeholder {
      color: #484f58; font-size: 13px; text-align: center; padding: 40px;
    }
    .relojes {
      width: 100%; max-width: 780px;
...
<body>
  ...
  <header>
    ...
    <div>
      <div class="badge offline" id="badge">Cargando...</div>
    </div>
  </header>
  ...
  <!-- El sismograma se carga solo si hay datos; si no, muestra mensaje de espera -->
  <div class="sismo-box" id="sismo-box">
    <div class="placeholder" id="placeholder">
      Esperando señal de la estación local…
    </div>
  ...
  <script>
    let hayDatos = false;   // controla si ya llegó alguna imagen

    function actualizarImg() {
      // Solo intenta cargar si el servidor reporta que está conectado
      if (!hayDatos) return;
      ...
      tmp.onerror = function() {
        // Imagen no disponible aún; no hace nada (mantiene el placeholder)
      };
      tmp.src = url;
    }
    ...
    function actualizarEstado() {
      fetch('/estado')
        ...
        .then(d => {
          ...
          document.getElementById('ts').textContent      = 'Última actualización de la estación: ' + d.hora;

          const b      = document.getElementById('badge');
          const banner = document.getElementById('banner');

          if (!d.conectado) {
            b.textContent = 'Estación Offline';
            b.className   = 'badge offline';
            banner.classList.remove('visible');
          } else if (d.evento_activo) {
            b.textContent = 'EVENTO';
          ...
          // Habilita la carga de imagen solo si el sensor está conectado
          if (d.conectado && !hayDatos) {
            hayDatos = true;
            actualizarImg();
          }
        })
